...
def image_demo(predictor, vis_folder, path, current_time, save_result, file_name):
...
def main(exp, args):
    ...
    time_weight_end = time.time()
    print(f'Time load weight model: {round(time_weight_end - time_weight_start, 3)} s')
    logger.info(f'Time load weight model: {round(time_weight_end - time_weight_start, 3)} s')
    ...
    if args.demo == "image":
        ...
        data_new = data.sample(20, random_state=42)
        total_time = 0
        for id in data_new.index:
            t0 = time.time()
            path = f'E:/WORKBASE/Project-rsna-breast-cancer-detection/DATA_STANDARD/DDSM/{data.view[id]}_{data.image_id[id]}.png'
            # par = data.patient_id[id]
            # lat = data.laterality[id]
            # view = data.view[id]
            # file_name = f"E:/WORKBASE/Project-rsna-breast-cancer-detection/META_DATA/Data_image/{par}/{view}/{lat}/{data.view[id]}_{data.laterality[id]}_{data.image_id[id]}.png"
            file_name = f'E:/WORKBASE/Project-rsna-breast-cancer-detection/DATA_STANDARD/DDSM_check/{data.view[id]}_{data.image_id[id]}.png'
        ...
        print(f"Mean time in a image = {total_time/len(data_new.index)}")
        # print(len(list_path))
        ...
        # t0 = time.time()
        # image_demo(predictor, vis_folder, args.path, current_time, args.save_result, file_name)
        # t = time.time() - t0
